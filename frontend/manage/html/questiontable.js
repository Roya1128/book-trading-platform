...
export default function prodpage(){
    Request().get("/index.php?action=getContacts")
    .then(res => {
        ...
        switch(response['status']){
            case 200:
                ...
                rows.forEach(element => {
                     ...
                     str += `<td name='id'>` + element['subject'] + `</td>`;
                     str += `<td>` + element['content'] + `</td>`;
                     str += `<td name=dates>` + element['datetime'] + `</td>`;
        ...
        `;
                    ...
                    for(let i=0; i<deleteButtons.length; i++){
                        deleteButtons[i].onclick = function(){
                            doDelete(null,4,emails[i].innerText,dates[i].innerText);
                        };
                    }
